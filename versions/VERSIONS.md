...
## [v1.7.0] - 2026-04-23
### 타이틀 시스템 고도화 및 감성 폰트 대규모 확충 (v1.7.0)
- **감성 폰트 15종 탑재**: 유튜브 음악 채널에서 선호되는 나눔손글씨, 감자꽃체, 동해독도체 등 감성 필기/흘림체 중심의 한글 폰트 15종을 엄선하여 교체했습니다. (고딕 2종 유지)
- **타이틀 진입 애니메이션 추가**: 페이드 인 외에도 둥둥 떠다니기(Floating), 물결(Wave), 드라마틱 줌, 타이핑 효과, 안개 해제(Blurry) 등 다채로운 진입 및 유지 효과를 구현했습니다.
- **실시간 렌더링 최적화**: GPU 가속을 활용하여 애니메이션이 포함된 타이틀 렌더링 성능을 개선했습니다.
- **버전 업데이트**: 시스템 전반 버전 동기화 (`v1.7.0`).

## [v1.6.1] - 2026-04-23
### 오디오 페이드 인/아웃 실시간 연동 수정 (v1.6.1)
- **실시간 반응 보정**: `VideoPlayer` 컴포넌트에서 페이드 인/아웃 설정값이 변경되었을 때 렌더링 루프가 이를 즉시 인지하여 오디오 볼륨 조절에 반영하도록 `useEffect` 의존성 배열을 수정했습니다.
- **버전 업데이트**: 시스템 전반 버전 동기화 (`v1.6.1`).

## [v1.6.0] - 2026-04-23
### 영상 미리보기 가독성 개선 (v1.6.0)
- **전체보기(Fit) 모드 적용**: 숏츠(9:16) 및 틱톡 영상 미리보기 시 배경 이미지가 잘려 가사가 가려지던 문제를 해결하기 위해 `object-cover`에서 `object-contain`으로 변경했습니다. 이제 영상의 모든 영역과 자막이 미리보기 화면에서 완벽하게 보입니다.
- **버전 업데이트**: 시스템 전반 버전 동기화 (`v1.6.0`).

## [v1.5.9] - 2026-04-23
### 이미지 히스토리 UI 개선 및 게시판 형태 도입 (v1.5.9)
- **리스트 스타일 변경**: 기존 버튼 나열 방식에서 세로형 게시판 리스트 형태로 변경하여 가독성을 높였습니다.
- **이미지 개수 표기**: 각 곡 제목 옆에 생성된 이미지 개수를 `[N]` 형식으로 표기하여 작업 상태를 즉시 확인할 수 있도록 개선했습니다.
- **디자인 고도화**: 외곽 박스 테두리를 삭제하고 마우스 호버 시 배경색 및 선형 하이라이트가 나타나는 모던한 리스트 디자인을 적용했습니다.
- **버전 동기화**: `package.json`, `App.tsx`, `LandingPage.tsx` 전면 적용 (`v1.5.9`).

## [v1.5.8] - 2026-04-23
### 이미지 생성 기록 누락 방지 및 데이터 최적화 (v1.5.8)
- **데이터 구조 최적화**: 히스토리(`sunoTracks`) 저장 시 중복된 base64 데이터(`localUrl`)를 제거하여 저장 용량 확보 및 Firestore 1MB 제한 충돌 방지.
- **LocalStorage 안정성 강화**: 히스토리 저장 시 `QuotaExceededError` 핸들링 추가. 용량 초과 시 오래된 기록의 이미지를 자동으로 삭제(Pruning)하여 최신 기록이 유실되지 않도록 보장.
- **이미지 생성 기록(History) 즉시 반영**: `generateAndSave` 프로세스 중 각 이미지가 생성될 때마다 히스토리 상태를 즉시 업데이트하도록 로직 개선.
- **버전 동기화**: `package.json`, `App.tsx`, `LandingPage.tsx` 전면 적용 (`v1.5.8`).

## [v1.5.7] - 2026-04-23
### 백업 관리 체계 정상화 및 규정 준수 강화 (v1.5.7)
- **백업 디렉토리 이동**: `EXECUTION_RULES.md` 제8조에 맞춰 `backups/` 폴더를 루트에 생성하고 기존 임시 백업 파일을 이동했습니다.
- **환경 설정 최적화**: `.gitignore`에 `backups/`를 명시하여 보안 및 관리 효율성을 높였습니다.
- **버전 동기화**: `package.json`, `App.tsx`, `LandingPage.tsx` 등 시스템 전반의 버전을 `v1.5.7`로 상향 조정했습니다.
