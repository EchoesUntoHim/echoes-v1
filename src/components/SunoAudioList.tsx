                ...
                try {
                    ...
                    if (userDoc.exists()) {
                        ...
                        if (cloudTracks.length > 0) {
                            ...
                            if (cloudTracks.length > 0 && !selectedTrackId) setSelectedTrackId(cloudTracks[0].id);
                    ...
                    addLog("⚠️ 클라우드 동기화 실패. 로컬 데이터를 사용합니다.");
                }
            }

            // Priority 2: Local Storage
            const savedData = localStorage.getItem('suno_json_data');
            if (savedData) {
                try {
                    const data = JSON.parse(savedData);
                    if (Array.isArray(data)) {
                        setTracks(data);
                        if (data.length > 0 && !selectedTrackId) setSelectedTrackId(data[0].id);
                    }
                } catch (e) {}
            }
        };
