  ...
  const generateYoutubeMetadata = async () => {
    ...
    try {
      ...
      const prompt = `
        ...
        {
          "title": "2026년 유튜브 알고리즘 최적화 및 타겟 시청자의 검색 의도를 관통하는 후킹한 제목",
          "description": "다음 구조와 줄바꿈(\\n\\n)을 반드시 엄격하게 준수하여 생성하세요:\\n\\n[첫 인사 및 곡 제목 소개 (🌿 이모지 포함)]\\n\\n[곡에 대한 은혜롭고 감성적인 설명 2-3줄]\\n\\n[시청자를 위한 기도와 축복의 메시지 (🙏 이모지 포함)]\\n\\n[🔔 구독과 좋아요 안내 문구 (💖 이모지 포함)]\\n\\n[유튜브/구글 검색 노출을 위한 관련 핵심 키워드 해시태그 10개]\\n\\n※ 각 섹션 사이에는 반드시 두 번의 줄바꿈(\\n\\n)을 넣어 가독성을 극대화하세요. 바로 복사해서 붙여넣기 좋게 생성해야 합니다.",
          "tags": "2026년 최신 유튜브 트렌드와 검색 데이터(SEO)를 기반으로 한 고노출 키워드 15개를 쉼표로 구분하여 생성하세요. (예: 은혜로운찬양, 가사있는찬양, CCM추천, 플레이리스트, ${workflow.params.mood}음악, ${workflow.params.topic} 등 실제 검색량이 검증된 단어 위주)"
        }
      `;
