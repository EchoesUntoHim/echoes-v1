...
match = re.search(r'const generateBlogPost = async \(\) => \{.*?generateLyrics = async \(\) => \{', content, re.DOTALL)
if match:
    full_old_func = match.group(0)
    actual_old_func = full_old_func[:full_old_func.rfind('  const generateLyrics = async')]
    new_content = content.replace(actual_old_func, new_code)
