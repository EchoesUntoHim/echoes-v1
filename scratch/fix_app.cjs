...
const broken_str = '        parsed = JSON.parse(cle      const prompt = `';

const fixed_str = `        parsed = JSON.parse(cleanedText);
      } catch (e) {
  ...
  const generateBlogPost = async () => {
    ...
    addLog('블로그 포스팅(네이버/티스토리/구글) 생성을 시작합니다...');
...
const old_state = `      blogSettings: {
        style: '감성 에세이형 (서정적, 감각적, 여운)',
        youtubeLink: ''
      },`;
      
const new_state = `      blogSettings: {
        style: '감성 에세이형 (서정적, 감각적, 여운)',
        youtubeLink: '',
