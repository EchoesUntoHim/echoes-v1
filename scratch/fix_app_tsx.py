...
# Fix the merged line and the whole function
# The merged line looks like: ... 의도가 우선입니다.' : ''      const naverPersona = `

merged_pattern = r"\'2\. \*\*以묒슂\*\*: ?ъ슜?먭? ?쒓났??\[?ъ슜??怨??댁꽍\]??理쒖슦?좎쑝濡?諛섏쁺?섏뿬 硫뷀??곗씠?곗쓽 諛⑺뼢?깆쓣 ?≪쑝?몄슂\. AI???먯쓽???댁꽍蹂대떎 ?ъ슜?먯쓽 ?섎룄媛€ ?곗꽑?낅땲??\' : \'\'      const naverPersona ="

# Replace with proper ending and function header
replacement_merged = r"'2. **중요**: 사용자가 제공한 [사용자 곡 해석]을 최우선으로 반영하여 메타데이터의 방향성을 잡으세요. AI의 임의적 해석보다 사용자의 의도가 우선입니다.' : ''
      `;
...
# This is a bit risky. Let's try to find the start of generateYoutubeMetadata and end at generateLyrics
# and replace everything in between with the correct two functions.

full_start_pattern = r'const generateYoutubeMetadata = async \(\) => \{'
full_end_pattern = r'const generateLyrics = async \(\) => \{'

match = re.search(full_start_pattern + r'.*?' + full_end_pattern, content, re.DOTALL)
if match:
    full_block = match.group(0)
    # We want to keep generateLyrics header
    actual_block = full_block[:full_block.rfind('  const generateLyrics = async')]
...
'''
    new_content = content.replace(actual_block, new_code)
